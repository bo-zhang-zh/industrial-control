...
#define CANVAS_X_SIZE   128
#define CANVAS_Y_SIZE   128
...
static const GUI_WIDGET_CREATE_INFO _aDialogCreate[] = {
  ...
  { BUTTON_CreateIndirect, "Button", ID_BUTTON_0, 230, 163, 80, 20, 0, 0, 0 },
  { EDIT_CreateIndirect, "Edit", ID_EDIT_0, 230, 96, 81, 20, 0, 100, 0 },
  { TEXT_CreateIndirect, "Text", ID_TEXT_0, 193, 97, 39, 20, 0, 100, 0 },
  { BUTTON_CreateIndirect, "Button", ID_BUTTON_1, 231, 123, 80, 20, 0, 0, 0 },
  { BUTTON_CreateIndirect, "Button", ID_BUTTON_2, 13, 179, 33, 20, 0, 0, 0 },
  { BUTTON_CreateIndirect, "Button", ID_BUTTON_3, 50, 179, 33, 20, 0, 0, 0 },
  { BUTTON_CreateIndirect, "Button", ID_BUTTON_4, 88, 179, 34, 20, 0, 0, 0 },
  { BUTTON_CreateIndirect, "Button", ID_BUTTON_5, 128, 180, 34, 20, 0, 0, 0 },
  { BUTTON_CreateIndirect, "Button", ID_BUTTON_6, 166, 179, 34, 20, 0, 0, 0 },
  { BUTTON_CreateIndirect, "Button", ID_BUTTON_7, 13, 206, 33, 20, 0, 0, 0 },
  { BUTTON_CreateIndirect, "Button", ID_BUTTON_8, 49, 206, 34, 20, 0, 0, 0 },
  { BUTTON_CreateIndirect, "Button", ID_BUTTON_9, 88, 206, 34, 20, 0, 0, 0 },
  { BUTTON_CreateIndirect, "Button", ID_BUTTON_10, 128, 206, 34, 20, 0, 0, 0 },
  { BUTTON_CreateIndirect, "Button", ID_BUTTON_11, 166, 206, 34, 20, 0, 0, 0 },
  { BUTTON_CreateIndirect, "Button", ID_BUTTON_12, 229, 203, 80, 20, 0, 0, 0 },
...
//	U8 sample[64] = {0};
  ...
  for (y = 0, yMin = 0; y < ySize; y += 4, yMin++) {
    for (x = 0, xMin = 0; x < xSize; x += 4, xMin++) {
      U8 Index;
      Offset = x >> 3;
      Bit    = 7 - (x & 7);
      Index  = *(pData + Offset);
...
//  #endif
  ...
  hMinImg = WM_CreateWindowAsChild(140, 5, 32, 32, hDlg, WM_CF_SHOW | WM_CF_STAYONTOP, 0, 0);
  WM_SelectWindow(hMinImg);
...
//  GUI_DrawRect(5, 5, 5+128, 5+128);
  while (1) {
	if (GUI_TOUCH_GetState(&tchPos) && PointIndex < 50)
	{
	  if (bFirstPoint)
	  {
		  xPos = tchPos.x;
		  yPos = tchPos.y;
		  if ((xPos >= CANVAS_X_POS) && (xPos <= CANVAS_X_POS+CANVAS_X_SIZE) 
				     && (yPos >= CANVAS_Y_POS) && (yPos <= CANVAS_Y_POS+CANVAS_Y_SIZE))
			{
	  ...
	  {
		  if (tchPos.x != xPos  || tchPos.y != yPos)
		  {
		    if ((xPos >= CANVAS_X_POS) && (xPos <= CANVAS_X_POS+CANVAS_X_SIZE) 
				     && (yPos >= CANVAS_Y_POS) && (yPos <= CANVAS_Y_POS+CANVAS_Y_SIZE))
			  {
