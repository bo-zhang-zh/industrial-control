...
void sdcard_timerproc(void *parameter);
...
struct dfs_partition sd_part;
struct rt_timer mci_timer;
...
static rt_size_t rt_sdcard_read(rt_device_t dev, rt_off_t pos, void* buffer, rt_size_t size)
{
	...
	status = sdcard_read(0, buffer, sd_part.offset + pos, size);
	if (status != RES_OK)
	{
...
static rt_size_t rt_sdcard_write (rt_device_t dev, rt_off_t pos, const void* buffer, rt_size_t size)
{
	...
	status = sdcard_write(0, buffer, sd_part.offset + pos, size);
	if (status != RES_OK)
	{
...
{
	rt_timer_init(&mci_timer,"mcitim",sdcard_timerproc,0,10,RT_TIMER_FLAG_PERIODIC);
	rt_timer_start(&mci_timer);
	if (sdcard_initialize(0) == RES_OK)
	{
		...
		if (status == RES_OK)
		{
			...
			if (dfs_filesystem_get_partition(&sd_part, sector, 0) != 0)
			{
