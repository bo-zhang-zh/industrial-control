...
struct struct_CO_Data {
	...
	stopped_t stopped;
     //作为Slave，接收到Node_Reset命令调用(若应用定义此回调函数)，后将状态设为Initialisation
     void (*NMT_Slave_Node_Reset_Callback)(CO_Data*);
		 //作为Slave，接收到Communications_Reset命令调用(若应用定义此回调函数)，后重设NodeID并将状态设为Initialisation
